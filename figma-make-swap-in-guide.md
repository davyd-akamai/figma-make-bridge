...
- **Doubled padding or a stray background color around the page content right after adopting the shell** is leftover cruft, not a template bug — `DefaultCmPageTemplate`'s content slot already provides background/padding/max-width, but moving existing content into that slot doesn't automatically delete the content's *old* outer wrapper styling that used to provide the same thing. `figma-make-swap-in-prompt.md`'s Phase 1 now explicitly asks Make to strip that redundant styling and screenshot-check for it — if this still shows up, the shell phase wasn't done carefully enough, worth re-running just that phase rather than patching the symptom by hand.
