...
**Step 1 — enumerate before changing anything.** Scan the entire prototype's source for any existing UI element that has an equivalent among the components documented in Guidelines.md's "Component reference" section (buttons, form fields, containers, navigation, etc. — whatever's currently exported). List every match you find, with its file and a short description, before converting anything. **Finding one instance of a pattern in a file is evidence there may be more** — if a file has one `RadioGroup`/`Checkbox`/etc., explicitly re-scan that same file for every other occurrence of the same source component before finalizing the list, rather than moving on after the first match. A file can have multiple, structurally-separate instances of the same pattern (e.g. two unrelated `RadioGroup`s covering different form fields), and finding the first one doesn't guarantee the enumeration caught the rest.
